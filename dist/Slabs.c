...
)
{
  ...
  if (size < SlabsCommon_max_size - (size_t)1U)
    return ((bounded_pair_){ .x = idx1, .y = idx5 });
  else
  {
...
typedef struct bounded_pair_0_s
{
  size_t x;
  size_t y;
}
...
)
{
  ...
  size_t idx5_ = v;
  ArrayList_insert(md_region, idx1, idx, 0U);
  bounded_pair_0 idxs = { .x = idx, .y = idx5_ };
  *r1 = idxs.x;
  *r5 = idxs.y;
  return idxs;
}
