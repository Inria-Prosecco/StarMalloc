...
let rotate_left_right (#a: Type) (ptr: t a)
  ...
  (ensures (fun h0 ptr' h1 ->
    rotate_left_right_wdm (v_linked_tree ptr h0)
    == Some (v_linked_tree ptr' h1)
  ))
  =
  let h = get () in
  assert (size_of_tree (v_linked_tree ptr h) <= c);
  rotate_left_right_size (v_linked_tree ptr h);
  assert (size_of_tree (opt_get (rotate_left_right_wdm (v_linked_tree ptr h)))
