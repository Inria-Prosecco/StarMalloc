...
val intro_llist_nil (a:Type0) (p : a -> vprop)
  : Steel unit emp (fun _ -> llist p (null_t #a))
          (requires fun _ -> True)
          (ensures fun _ _ h1 -> v_llist #a p null_t h1 == [])

val elim_llist_nil (#a:Type0) (p : a -> vprop) (ptr:t a)
  : Steel unit (llist p ptr) (fun _ -> llist p ptr)
          (requires fun _ -> ptr == null_t)
          (ensures fun h0 _ h1 ->
            v_llist p ptr h0 == v_llist p ptr h1 /\
            v_llist p ptr h1 == [])

val cons_is_not_null (#a:Type0) (p : a -> vprop) (ptr:t a)
  : Steel unit (llist p ptr) (fun _ -> llist p ptr)
