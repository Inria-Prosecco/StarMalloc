...
type tree (a: Type) =
  | Leaf : tree a
  | Node: data: a -> left: tree a -> right: tree a -> size: I.t -> tree a
...
let rec size_of_tree (#a: Type) (x: tree a) : Tot I.t (decreases x) =
  match x with
  | Leaf -> zero
  | Node _ left right _ -> size_of_tree left + size_of_tree right + one

(* is with defined size *)
let rec is_wds (#a: Type) (x: tree a) : Tot (bool & I.t) (decreases x) =
  match x with
  | Leaf -> true, zero
...
let induction_wds (#a: Type) (x: a) (l r:wds a)
  : Lemma (let s = size_of_tree l + size_of_tree r + one in
           let t = Node x l r s in
   fst (is_wds t))
  =
  assert (fst (is_wds l));
  assert (fst (is_wds r));
  let s = size_of_tree l + size_of_tree r + one in
  let t = Node x l r s in
...
let rec height (#a: Type) (x: tree a) : I.t =
  match x with
  | Leaf -> zero
  | Node data left right _ ->
    let hleft = height left in
    let hright = height right in
    if gt hleft hright then hleft + one
    else hright + one

(**** Append *)
let aux_size_left_subtree (#a: Type) (t1: tree a) (t2: tree a)
  : Lemma
  (requires (size_of_tree t2 == size_of_tree t1 + one))
  (ensures (
    forall (x:a) (tr: tree a) (n:I.t).
    size_of_tree (Node x t2 tr n) == size_of_tree (Node x t1 tr n) + one))
  = admit ()
...
let aux_size_right_subtree (#a: Type) (t1: tree a) (t2: tree a)
  ...
  (requires (size_of_tree t2 == size_of_tree t1 + one))
  (ensures (
    forall (x:a) (tl: tree a) (n:I.t).
    size_of_tree (Node x tl t2 n) == size_of_tree (Node x tl t1 n) + one))
  = admit ()
...
let rec append_left_aux (#a: Type) (t: wds a) (v: a)
  : tree a
  = match t with
  | Leaf -> Node v Leaf Leaf one
  | Node x left right size ->
    Node x (append_left_aux left v) right (size + one)

let rec append_left_aux_size (#a: Type) (t: wds a) (v: a)
  : Lemma (size_of_tree (append_left_aux t v) == size_of_tree t + one)
  = match t with
  | Leaf -> ()
  | Node x left right size ->
...
let rec append_left_aux_size2 (#a: Type) (t: wds a) (v: a)
  ...
  | Node x left right size ->
      ...
      append_left_aux_size left v;
      assert (size_of_tree new_left == size_of_tree left + one);
...
let rec append_right_aux (#a: Type) (t: tree a) (v: a)
  ...
  | Leaf -> Node v Leaf Leaf one
  | Node x left right size ->
    Node x left (append_right_aux right v) (size + one)

let rec append_right_aux_size (#a: Type) (t: wds a) (v: a)
  : Lemma (size_of_tree (append_right_aux t v) == size_of_tree t + one)
  = match t with
  | Leaf -> ()
  | Node x left right size ->
...
let rec append_right_aux_size2 (#a: Type) (t: wds a) (v: a)
  ...
  | Node x left right size ->
      ...
      append_right_aux_size right v;
      assert (size_of_tree new_right == size_of_tree right + one);
...
let rec is_balanced (#a: Type) (x: tree a) : bool =
  ...
  | Leaf -> true
  | Node data left right _ ->
    I.lte (height right) (one + height left) &&
    I.lte (height left) (one + height right) &&
    is_balanced(right) &&
    is_balanced(left)
...
let rotate_left (#a: Type) (r: tree a) : option (tree a) =
  match r with
  | Node x t1 (Node z t2 t3 _) _ -> Some (Node z (Node x t1 t2 zero) t3 zero)
  | _ -> None

let rotate_left_wds (#a: Type) (r: wds a) : option (wds a) =
  match r with
  | Node x t1 (Node z t2 t3 _) s ->
      let s12 = sot_wds t1 + sot_wds t2 + one in
      let t12 = Node x t1 t2 s12 in
      induction_wds x t1 t2;
      assert (fst (is_wds t12));
      assert (fst (is_wds r));
      check r;
      let s123 = s12 + sot_wds t3 + one in
      induction_wds z t12 t3;
      admit ();
      assert (I.eq s s123);
...
let rotate_right_left (#a: Type) (r: tree a) : option (tree a) =
  ...
  | Node x t1 (Node z (Node y t2 t3 _) t4 _) _ ->
      Some (Node y (Node x t1 t2 zero) (Node z t3 t4 zero) zero)
  | _ -> None
...
let rotate_right_left_wds (#a: Type) (r: wds a) : option (wds a) =
  ...
  | Node x t1 (Node z (Node y t2 t3 _) t4 _) s ->
      assume (fst (is_wds t1));
      assume (fst (is_wds t2));
      assume (fst (is_wds t3));
      assume (fst (is_wds t4));
      admit ();
      let s12 = sot_wds t1 + sot_wds t2 + one in
      let t12 = Node x t1 t2 s12 in
      induction_wds x t1 t2;
      assert (fst (is_wds t12));
      let s34 = sot_wds t3 + sot_wds t4 + one in
      let t34 = Node z t3 t4 s34 in
      induction_wds z t3 t4;
      assert (fst (is_wds t34));
      let s1234 = s12 + s34 + one in
      assert (s1234 == s);
...
let rotate_left_right (#a: Type) (r: tree a) : option (tree a) =
  match r with
  | Node x (Node z t1 (Node y t2 t3 _) _) t4 _  -> Some (Node y (Node z t1 t2 zero) (Node x t3 t4 zero) zero)
  | _ -> None
...
let rebalance_avl (#a: Type) (x: tree a) : tree a =
    ...
    | Node data left right _ ->
      if is_balanced x then x
        ...
        if gt (height left) (height right + one) then (
          let Node ldata lleft lright _ = left in
          if gt (height lright) (height lleft) then (
        ...
        ) else if gt (height right) (height left + one) then (
          let Node rdata rleft rright _ = right in
            if gt (height rleft) (height rright) then (
...
let rebalance_avl_wds (#a: Type) (t: wds a) : wds a =
  ...
  if gt (height left) (height right + one) then (
    ...
    if lte (height lleft) (height lright) then (
